...
import { ChevronLeft, ChevronRight, Star, Heart, MapPin, Flame, CheckCircle2, UserPlus, UserCheck } from 'lucide-react'
  ...
  return (
    <section className="w-full bg-market-bg py-5 px-3 sm:px-5 border-b border-market-border">
            ...
            return (
              ...
              >
                <div className="aspect-[3/4] relative overflow-hidden">
                  <img src={card.image} alt={card.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                  ...
                  <button onClick={e => toggleLike(e, card.id)} className="absolute top-2 right-2 w-7 h-7 flex items-center justify-center bg-black/50 hover:bg-black/70 rounded-full transition-all">
                    <Heart size={13} className={liked.has(card.id) ? 'text-[#E91E63] fill-[#E91E63]' : 'text-white'} />
                  </button>

                  <div className={`absolute bottom-[88px] left-2.5 flex items-center gap-1.5 px-2 py-1 rounded-full text-[10px] font-medium ${card.available ? 'bg-[#28a745]/20 text-[#28a745] border border-[#28a745]/30' : 'bg-white/10 text-white/60'}`}>
      ...
      <div className="flex justify-center gap-1.5 mt-3">
        {FEATURED.map((_, i) => (
          <button key={i} onClick={() => { setIdx(i); setPaused(true) }} className={`h-1.5 rounded-full transition-all duration-300 ${i === idx ? 'bg-secondary-color w-5' : 'bg-text-muted/40 w-1.5'}`} />
        ))}
      </div>
    </section>
  )
