...
-- PRODUCTION: Do NOT insert a default admin account here.
-- After importing the schema, create the admin manually:
--   1. Generate a hash:  node -e "const{scrypt,randomBytes}=require('crypto');const{promisify}=require('util');const salt=randomBytes(16).toString('hex');promisify(scrypt)('YourStrongPass',salt,64).then(h=>console.log(salt+':'+h.toString('hex')))"
--   2. INSERT INTO users (username,email,password_hash,role,display_name) VALUES ('admin','admin@wet3.camp','<hash>','admin','Site Administrator');
--
-- DEVELOPMENT ONLY (remove before production import):
